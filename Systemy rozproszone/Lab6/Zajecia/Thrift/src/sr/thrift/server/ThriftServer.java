...
public class ThriftServer {
	...
	public static void simple() 
	{
		try {
			...
			TServer server1 = new TSimpleServer(new Args(serverTransport1).protocolFactory(protocolFactory)
					.processor(processor1)
			);
			TServer server2 = new TSimpleServer(new Args(serverTransport2).protocolFactory(protocolFactory).processor(processor2));
			TServer server3 = new TSimpleServer(new Args(serverTransport3).protocolFactory(protocolFactory).processor(processor3));

			System.out.println("Starting the simple server...");

			Runnable runnable1 = () -> server1.serve();
			Runnable runnable2 = () -> server2.serve();
